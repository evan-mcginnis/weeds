...
% A hack to remove the blank page before the title page
...
\newacronym{EXGEXR}{ExGExR}{Excess Green - Excessive Red}
...
\newacronym{CIELab}{CIELab}{A color space defined by the International Commission on Illumination (CIE)}
\newacronym{CLAHE}{CLAHE}{Contrast Limited Adaptive Histogram Equalization}
\newacronym{UAV}{UAV}{Unmanned Aerial Vehicle (often called a \textit{drone})}
...
%  \vhEntry{2.0}{03 Apr 2025}{EM}{Changes discussed during interactive review}
%  \vhEntry{2.1}{06 Apr 2025}{EM}{Incorporate review comments on V1.9}
%  \vhEntry{2.2}{07 Apr 2025}{EM}{Incorporate review comments for Chapters 1 \& 2}
%\end{versionhistory}
%\newpage
%}
...
Operational precision weed treatment in crops requires accurate classification of plants as either desired or undesired. This dissertation presents findings on how to accurately and efficiently distinguish weeds from crops using shape, color, and texture all of which can be extracted from RGB images.  Nine different binary classification approaches were examined: Decision Tree, Extra Trees, Gradient Boosting, \gls{KNN}, \gls{LDA}, Logistic Regression, \gls{MLP}, Random Forest, and \gls{SVM}. Additionally, use of a unary classification approach was considered. Two sets of crop images were used in  this work: broccoli and cantaloupe. An \gls{AUC}, a measure of accuracy, of 0.925 was obtained with a Random Forest classification using combination of shape (\textit{roundness}, \textit{convexity}), color (\textit{saturation mean}), and texture (\textit{hog mean}).  While the concept of the shape of an object is familiar to many, the texture attributes are not, especially when expressing a 3 dimensional concept from a 2 dimensional image. The textural attributes are computed for the objects in the image in non-RGB color spaces. Specifically, the texture attributes are drawn from channels in the \gls{HSI}, YCbCr, \gls{CIELab} color space, as well as the more typical greyscale conversion. Using only texture attributes resulted in an AUC of 0.90 (Random Forest), indicating that texture alone can be used in prediction with reasonably good performance. Using only color produced a model with an AUC of 0.87 (Random Forest), and only shape in classification resulted in an \gls{AUC} of 0.86 (Boosted Gradient). These values were derived from images of the vegetation acquired over several weeks early in the development cycle. Higher AUC values are obtained when using images from a single day: an AUC of 0.97 was obtained using Extra Trees classification on images acquired four weeks into the broccoli development cycle. Some of the increase seen can be explained by the factors exhibiting the most changes across the development cycle. A Dickey-Fuller test ($\alpha = 0.05$), a statistical test used to determine if a time series is stationary by testing for the presence of a unit root, shows that the \textit{hue} of vegetation is stable over the development cycle, but some shape attributes---such as the \textit{radial variance}--- are not. A significant test result (p-value less than $0.05$) suggesting the time series is stationary, meaning it does not exhibit trends or persistent patterns over time.
...
Class imbalance correction using oversampling algorithms \gls{SMOTE}, borderline, K-Means, \gls{ADASYN}, \gls{SVM} as well as combined oversampling/undersampling algorithms of SMOTE+\gls{ENN} and SMOTE+TOMEK were also evaluated. For oversampling approaches Decision Tree benefited the most, Random Forest the least. For the combined approach, Decision Tree once again benefited the most, however this technique led to much lower $F_{1}$ scores from Logistic Regression (SMOTE+ENN) and Random Forest (SMOTE+TOMEK).

These findings demonstrate that calculating texture features in non-RGB color spaces, such as \gls{HSV}, YIQ, and CIELab, yields superior classification accuracy compared to traditional grayscale-based approaches. By leveraging color-separated texture analysis, particularly using \gls{GLCM} and related descriptors, the study highlights improved differentiation between crops and weeds in early developmental stages. Results indicate that texture patterns extracted from hue and chromaticity channels provide richer discriminative information than grayscale intensity alone, leading to higher precision and recall in classification models. Generally, our findings suggest that integrating texture analysis from alternative color spaces enhances weed/crop classification performance, offering a more robust solution for early-season weed treatment automation.

This work explores a weed/crop classification framework tailored for precision agriculture applications. The ultimate goal is to develop a system that can be optimized and seamlessly integrated into a ground-based treatment process, enabling more efficient weed management. Drawing on extensive experimental results, this paper presents findings that support the feasibility of achieving that integrated approach. Overall, these outcomes demonstrate a promising path forward toward realizing a high-performance, adaptable weed/crop classification solution.
...
\setstretch{1.0}
% Example to span two pages
\begin{longtable}{x{\dimexpr.23\columnwidth-2\tabcolsep}
                  x{\dimexpr.23\columnwidth-2\tabcolsep}
                  x{\dimexpr.23\columnwidth-2\tabcolsep}
                  x{\dimexpr.23\columnwidth-2\tabcolsep}}
%\begin{hyphenrules}{nohyphenation}
...
% Original paragraph
%Each image acquisition activity was accompanied by an image of a \href{https://www.datacolor.com/spyder/products/spyder-checkr-photo/} {SpyderChecker 24} color calibration chart (under the same ambient lighting conditions) and color correction was subsequently applied using Adobe Photoshop Lightroom Classic and custom calibration profiles created using the Spyder Checkr software. Raw camera sensor data from both camera systems was stored in Adobe Digital Negative (DNG) format before color correction and conversion to a JPG format. The digital negative of each image (DNG, a standard format for raw camera sensor data) was corrected and converted to JPG format for subsequent processing. The images in all digital cameras start out as the values from a sensor; many cameras simply provide this in a convenient to use format, and JPG is the most widely used. We note here that a format like JPG does not actually represent what the sensor  has ``collected'', but is the result of the in-camera processing of sensor data. In general this process is consistent preserving the sensor data fidelity across time and space. Color correction, for instance, is easier with raw sensor data than a JPG due to  the much wider color spectrum present in the raw sensor data (16.7 million vs 68.7 billion is a big difference) \parencite{AdobeUnknown-re}. The camera sensor data for each image was quite large, requiring 38.5 MB of storage, with equivalent JPG images much smaller.
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.6\linewidth]{./figures/color-correction.jpg}
...
%Consider the size of the ground captured with a \gls{UAV} flight at various heights:
%
...
% This will produce a 2x2 grid on a standard letter-sized page
% This produces figures that have aligned captions -- the [t] bit does the trick
\begin{figure}[H]
	\centering
	\subfloat[Leaf with reflection]{\includegraphics[width=.44\linewidth]{figures/reflection.jpg}\label{fig:leaf-with-reflection}}
	\hfill
	\subfloat[Before]{\includegraphics[width=.44\textwidth]{figures/reflection-histogram.jpg}\label{fig:reflection-histogram}}
	\hfill
	\subfloat[After equalization]{\includegraphics[width=.44\textwidth]{figures/reflection.equalized.jpg}\label{fig:leaf-equalized}}
	\hfill
	\subfloat[After equalization]{\includegraphics[width=.44\textwidth]{figures/reflection-histogram-equalized.jpg}\label{fig:leaf-equalized-histogram}}
	\caption[Employing histogram equalization]{Histogram equalization can be used to address varying illumination problems that are commonly found in ambient light images.}
...
\begin{figure*}[h]
	\centering
	  	\subfloat[Intensity Histogram\centering]{\includegraphics[height=0.23\textheight]{figures/pixel-values.png}\label{fig:intensity-histogram}}
		\hfil
	   	\subfloat[Source Image\centering]{\includegraphics[height=0.23\textheight]{figures/pixel-values-image.jpg}
...
% To make the figure larger, we will need to ditch the side-b-side view.
%\begin{figure}[h!]
%	\centering
%	\begin{subfigure}[h]{.49\textwidth}
%	  \centering
%	  \includegraphics[height=7cm]{./figures/ndi-1-of-2.png}
...
%	\begin{subfigure}[h]{.49\textwidth}
%	  \centering
%	  \includegraphics[height=7cm]{./figures/triangle-algorithm}
...
\subfloat[NDI]{\includegraphics[height=0.12\textheight]{figures/20201117_112624-NDI.jpg} \label{fig:ndi}} &
\subfloat[EXG]{\includegraphics[height=0.12\textheight]{figures/20201117_112624-ExG.jpg} \label{fig:exg}} &
\subfloat[EXR]{\includegraphics[height=0.12\textheight]{figures/20201117_112624-ExR.jpg} \label{fig:exr}} &
\subfloat[CIVE]{\includegraphics[height=0.12\textheight]{figures/20201117_112624-CIVE.jpg} \label{fig:cive}} &
\subfloat[EXG-EXR]{\includegraphics[height=0.12\textheight]{figures/20201117_112624-ExGR.jpg} \label{fig:exgexr}} &
\subfloat[NGRDI]{\includegraphics[height=0.12\textheight]{figures/20201117_112624-NGRDI.jpg} \label{fig:nrgdi}} \\
\subfloat[Com1]{\includegraphics[height=0.12\textheight]{figures/20201117_112624-COM1.jpg} \label{fig:com1}} &
\subfloat[Com2]{\includegraphics[height=0.12\textheight]{figures/20201117_112624-COM2.jpg} \label{fig:com2}}&
\subfloat[MEXG]{\includegraphics[height=0.12\textheight]{figures/20201117_112624-MexG.jpg} \label{fig:mexg}} &
\subfloat[TGI]{\includegraphics[height=0.12\textheight]{figures/20201117_112624-TGI.jpg} \label{fig:tgi}} &
\subfloat[VEG]{\includegraphics[height=0.12\textheight]{figures/20201117_112624-VEG.jpg} \label{fig:veg}} &
%\subfloat[Original]{\includegraphics[scale=0.0255,angle=90]{figures/20201117_112624.jpg} \label{fig:original}} \\
\subfloat[Original]{\includegraphics[scale=0.0255,angle=270]{figures/20201117_112624-original.jpg} \label{fig:original}} \\
...
The processing employed here is to convert the image to the YIQ color space and take the mean value for the $I$, or in-phase component for the blob's pixels. (\cite{MathWorks2023-xp})
\begin{figure}[h!]
	\centering
...
%
%Table \ref{table:ml-segmentation} shows a summary of the three classification approaches in various color spaces, giving the Area Under the Curve (AUC), Precision, Recall, and $F_{1}$ scores. And while the  scores reported are encouraging, the time taken to produce the resulting segmented images is not. Some of the techniques required hours to process a high-resolution image. Even with optimization and reduced-resolution images, it is unlikely that any of these classification techniques would be appropriate for use in a real-time system.
%
...
%\toprule
%Technique &  Color &  AUC & Precision & Recall &   $F_{1}$\\
%\midrule
%\endfirsthead
...
\begin{figure*}[!htb]
	\centering
	\subfloat[Original\centering]{\includegraphics[width=4cm]{./figures/transect-image.jpg}\label{fig:transect-original-hsi}}
	\hfill
...
\begin{figure*}[!htb]
	\centering
	\subfloat[Original\centering]{\includegraphics[width=4cm]{./figures/transect-image.jpg}\label{fig:transect-original-hsv}}
	\hfill
...
\begin{figure*}[h]
	\centering
	\subfloat[Original\centering]{\includegraphics[width=4cm]{./figures/transect-image.jpg}\label{fig:transect-original-cielab}}
	\hfill
...
\begin{figure*}[h]
	\centering
	\subfloat[Original\centering]{\includegraphics[width=4cm]{./figures/transect-image.jpg}\label{fig:transect-original-ycbcr}}
	\hfill
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{./figures/hog.jpg}
...
\captionsetup[subfloat]{labelfont=scriptsize,textfont=scriptsize}
\begin{tabular}{cc}
\subfloat[Cantaloupe \& Weed]{\includegraphics[height=0.25\textheight, angle=90]{figures/result-1-raw-00003.jpg} \label{fig:ndi}} &
\subfloat[Classification]{\includegraphics[height=0.25\textheight, angle=90]{figures/result-1-processed-com2-lr-00003.jpg} \label{fig:exg}} \\
\subfloat[Broccoli \& Weed]{\includegraphics[height=0.25\textheight, angle=90]{figures/result-2-raw-00097.jpg} \label{fig:result-2-raw}} &
\subfloat[Classification]{\includegraphics[height=0.25\textheight, angle=90]{figures/result-2-processed-ycbcri-lr-00097.jpg} \label{fig:result-2-classified}} \\
\subfloat[Cantaloupe]{\includegraphics[height=0.18\textheight]{figures/result-3-raw-00000.jpg} \label{fig:result-3-raw}} &
\subfloat[Classification]{\includegraphics[height=0.18\textheight]{figures/result-3-processed-com2-lr-00000.jpg} \label{fig:result-3-classified}} \\
\subfloat[Weed in cantaloupe]{\includegraphics[height=0.18\textheight]{figures/result-4-raw-00023.jpg} \label{fig:result-4-raw}} &
\subfloat[Classification]{\includegraphics[height=0.18\textheight]{figures/result-4-processed-com2-lr-00023.jpg} \label{fig:result-4-classified}} \\
\end{tabular}
...
% Generated with ./create-dissertation-tex tables

\input{./generated/tables/generated-results-texture.tex}
...
{
\input{./generated/tables/generated-details-results-texture.tex}
}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\linewidth]{./generated/figures/generated-roc-texture.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\linewidth]{./generated/figures/generated-roc-texture-occ.png}
...
%\subsection{GLCM and LBP}
%The features used in binary classification used primarily \gls{GLCM} and \gls{HOG}. Consider the results when using these features exclusively, as Figure \ref{fig:glcm-lbp} demonstrates.
%
%\begin{figure}[H]
%	\centering
%	\begin{subfigure}[h]{0.45\linewidth}
%		\centering
%		\includegraphics[width=6cm]{./generated/figures/generated-roc-glcm.png}
%		\caption{Binary Classification with GLCM}
%		\label{subfig:glcm-roc}	
%	\end{subfigure}
%	\begin{subfigure}[h]{0.45\linewidth}
%		\centering
%		\includegraphics[width=6cm]{./generated/figures/generated-roc-lbp.png}
%		\caption{Binaru Classification with LBP}
%		\label{subfig:lbp-roc}		
%	\end{subfigure}%
%	\caption[Binary Classification with GLCM and LBP]{Binary Classification with GLCM and LBP}
%	\label{fig:glcm-lbp}
%\end{figure}
...
%% Reduce the row spacing, so to table looks a bit more packed
%\renewcommand{\arraystretch}{1.2}
%\input{./generated/tables/generated-results-glcm.tex}
%\input{./generated/tables/generated-details-results-glcm.tex}
%\end{small}
...
%Classifying using LBP yields much better results for \textit{Logistic Regression,} with an AUC of $0.872$, but some techniques yielded identical results. Most techniques, however yielded higher results.
%\begin{small}
%\renewcommand{\arraystretch}{1.2}
%\input{./generated/tables/generated-results-lbp.tex}
%\input{./generated/tables/generated-details-results-lbp.tex}
%\end{small}
...
\input{./tables/revised-results-texture-color-greyscale-comparison-all-metrics.tex}
}
...
\input{./generated/tables/generated-seasonality-texture.tex}
...
\input{./generated/tables/generated-results-color.tex}
\input{./generated/tables/generated-details-results-color.tex}
}
%\end{tiny}
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.8\linewidth]{./generated/figures/generated-roc-color.png}
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.8\linewidth]{./generated/figures/generated-roc-color-occ.png}
...
{
\renewcommand{\arraystretch}{1.2}
\input{./generated/tables/generated-seasonality-color.tex}
}
...
{
\input{./generated/tables/generated-results-shape.tex}
}
...
\input{./generated/tables/generated-details-results-shape.tex}
}

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.8\linewidth]{./generated/figures/generated-roc-shape.png}
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.8\linewidth]{./generated/figures/generated-roc-shape-occ.png}
...
% make optimal TRAINING=/cygdrive/d/dissertation/shape/season.csv FACTORS=7 PREFIX=shape TYPE=SHAPE SUBTYPE=all OUTPUT=/cygdrive/d/dissertation/shape 
%  make optimal-results PREFIX=shape PARAMETERS=./parameters.pickle RESULTS=/cygdrive/d/dissertation/shape/ TYPE=SHAPE SUBTYPE=all CAPTION_LONG="Classify by shape" CAPTION_SHORT="Classify by shape" LABEL=table:results-shape FORMAT=latex OUTPUT=../documents/tables/results-shape.tex
\input{./generated/tables/generated-seasonality-shape.tex}
}
...
% make optimal TRAINING=/cygdrive/d/dissertation/shape/season.csv FACTORS=7 PREFIX=shape TYPE=SHAPE SUBTYPE=all OUTPUT=/cygdrive/d/dissertation/shape 
% make optimal-results TYPE=all SUBTYPE=all FORMAT=latex PARAMETERS=parameters.pickle LOGGING=tucson-logging.ini ALL_RESULTS=/cygdrive/d/dissertation/debug/ CAPTION_LONG="Classification using all features" CAPTION_SHORT="Classification using all features" LABEL=table:optimal PREFIX=uncorrected_all_auc OUTPUT=../documents/tables/results-optimal.tex
\input{./generated/tables/generated-results-optimal.tex}
...
{
\input{./generated/tables/generated-details-results-optimal.tex}
}

% Produced with:
%make optimal-results TYPE=all SUBTYPE=all FORMAT=latex PARAMETERS=parameters.pickle LOGGING=tucson-logging.ini ALL_RESULTS=/cygdrive/d/dissertation-revision/ CAPTION_LONG="Classification using all features (revision)" CAPTION_SHORT="Classification using all features (revision)" LABEL=table:optimal PREFIX=all5_auc OUTPUT=../documents/tables/revised-results-optimal.tex COLUMNS="auc f1 precision recall"



%{
%\input{./tables/revised-results-optimal.tex}
%}

% Produced with:
% make optimal-results TYPE=all SUBTYPE=all FORMAT=latex PARAMETERS=parameters.pickle LOGGING=tucson-logging.ini ALL_RESULTS=/cygdrive/d/dissertation-revision/ CAPTION_LONG="Classification using all features (revision)" CAPTION_SHORT="Classification using all features (revision)" LABEL=table:optimal PREFIX=all5_auc OUTPUT=../documents/tables/revised-results-optimal-occ.tex COLUMNS="auc f1 precision recall" APPROACH=occ
...
%{
%% Originally, this showed only the AUC:
...
% Produced with
% make  optimal-counts TYPE=all SUBTYPE=all FORMAT=latex PARAMETERS=parameters.pickle LOGGING=tucson-logging.ini ALL_RESULTS=/cygdrive/d/maricopa-test/dissertation CAPTION_LONG="Percentage of Classifications within %T of Maximum (revised)" CAPTION_SHORT="Percentage of Classifications within %T of maximum (revised)" LABEL=table:results-counts PREFIX=dissertation_auc OUTPUT=../documents/tables/revised-results-counts.tex THRESHOLD=0.01

{
\input{./generated/tables/generated-results-counts.tex}
}



% Produced with
% python results.py --prefix  dissertation_auc -c parameters.pickle -o latex --type all --subtype all -lg tucson-logging.ini --directory d:/maricopa-test/dissertation/ --long "Receiver Operating Characteristic"  --short "Receiver Operating Characteristic" --label table:counts --summary --roc --graph ../documents/figures/roc.png --data d:/dissertation/shape/season.csv

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.99\linewidth]{./generated/figures/generated-roc-all.png}
...
% Produced with
%  python results.py --prefix  all5_auc -c parameters.pickle -o latex --type all --subtype all -lg tucson-logging.ini --directory d:/dissertation-revision/ --long "Thresholds for classification approaches"  --short "Thresholds for classification approaches" --label table:thresholds --summary --roc-thresholds --data d:/dissertation/shape/season.csv --technique binary --table ../documents/tables/thresholds.tex
{
\input{./generated/tables/generated-thresholds.tex}
}
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.99\linewidth]{./generated/figures/generated-roc-all-occ.png}
...
\input{./generated/tables/generated-seasonality-all.tex}
}
%
% MARICOPA 
%
...
%\label{table:xxxx}\\
%\toprule
%Technique &  Precision &  Recall &   $F_{1}$ &  MAP &  AUC \\
%\midrule
%\endfirsthead
...
%The Principal Component Analysis (\gls{PCA}) \parencite{Muller2016-ui} seen in Figure~\ref{fig:pca} shows most of the variance can be explained by only 3 features: \textit{hue}, \textit{saturation\_mean}, and \textit{in\_phase}.
%\begin{figure}[h]
%	\centering
%	\begin{subfigure}[h]{.48\textwidth}
%		  \centering
...
%		% It's that \gls{PCA} line that is the trouble, as the values are just too long
%		\footnotesize
%		
%		\begin{longtable}[c]{
%		    |l |*{12}{c |} }%
...
%Influence of color space. Table 4.3 provides a controlled comparison between gray–only co-occurrence
%matrices and those assembled separately in five color spaces. Every classifier except SVM posts a positive
%AUC delta; Decision Tree gains 3.9\%, Extra Trees 1.2\%. Three mechanisms explain the lift: (i) pigment
%gradients introduce chromatic co-variation unavailable in gray luminance, (ii) shadows modulate luminance
%more than chrominance, so chromatic GLCMs are less noise–prone, and (iii) the entropy of a three–channel
%neighborhood is higher, yielding a richer feature manifold. Notably, the improvements taper for highly
%regularized kernels, showing that color texture benefits learners capable of higher–order interactions.

%Binary vs. unary ROC detail. In Figure 4.8 the unary ROC curve of Extra Trees lies marginally
%below the binary curve between FPR 0.0–0.1 but converges thereafter. The AUC difference is 0.017—a gap
%that translates to three additional false weeds per hundred images at the spray/no–spray threshold used in
%Chapter 5. Whether that trade–off is acceptable depends on herbicide cost and collateral damage tolerance;
%the dissertation’s economic analysis in Chapter 6 suggests it often is.
...
{
\input{generated/tables/generated-parameter-counts-texture}
}
...
{
\input{./generated/tables/generated-parameter-counts-all.tex}
}
...
%\chapter{Candidate}
%
%This expanded discussion (approximately 4,000 words) situates the empirical findings from Chapter~4 in a broader agronomic and machine–learning context. We follow the structure of the \textit{Results} section, adding interpretive depth, cross–referencing tables and figures, and elaborating on theoretical implications. Particular emphasis is placed on the AUC statistic and its visualization through ROC curves, because those plots provide a threshold–independent perspective that unifies binary and unary experiments. We close with guidance for practitioners and suggestions for future work. All section, table, and figure numbers refer to the dissertation unless noted otherwise.
%
%\subsection{Binary \emph{versus} unary formulations}
%
%The dissertation evaluates every feature family in both a conventional binary setting (crop \emph{vs.} weed) and a one–class or ``unary’’ setting in which the model is trained only on crop exemplars and asked to flag deviations as weeds. Across 27 pairwise comparisons the binary learner posted the higher AUC in 21 cases, but the absolute gap was modest: a median of 0.018 and never more than 0.030. For texture, the binary ROC envelope in Figure4.7 hugs the y–axis more tightly than its unary counterpart in Figure4.8, yet both sets of curves rise well above the chance diagonal and plateau near the same upper boundary. The pattern replicates for color (Figures 4.13–4.14) and shape (Figures 4.15–4.16). In practice, this means that when annotated weeds are scarce—as is common at new production sites—the unary pipeline offers a viable fallback at the cost of a few hundredths of AUC. That conclusion generalizes across all classifiers, but tree ensembles degrade least, suggesting that their flexible partitioning of feature space approximates weed density well even without direct weed labels.
%
%\subsection{Texture‐based discrimination: GLCM, LBP, and HOG}
%
%\paragraph{Performance overview.}
%Table 4.2 ranks nine supervised learners on a 133–variable texture set combining per–channel GLCM, multi–scale LBP, and rudimentary HOG summaries. Extra Trees, Gradient Boosting, and Random Forest cluster at the top (AUC 0.89–0.90), while Decision Tree trails at 0.80. The ROC traces confirm that the strong learners achieve both a steep initial ascent (high sensitivity at low false–positive rates) and a broad asymptote (high specificity at relaxed thresholds). The union of HOG with second–order statistics proved synergistic: where pure GLCM baselines faltered on narrow monocots versus dicots, HOG captured long‐range venation patterns.
%
%\paragraph{GLCM versus LBP.}
%Although individual GLCM descriptors appear in \emph{every} high–scoring subset, none of the best models rely on GLCM alone. Random Forest, for example, elevates \texttt{cie\_b\_lbp\_variance} and \texttt{greyscale\_lbp\_stddev} alongside \texttt{greyscale\_contrast\_avg}, indicating that micro–texture (LBP) and meso–texture (GLCM) encode complementary spatial frequencies. KNN, conversely, selects four GLCM variables exclusively and suffers the largest AUC penalty, underlining the risk of omitting LBP for distance–based learners.
%
%\paragraph{Influence of color space.}
%Table 4.3 provides a controlled comparison between gray–only co‑occurrence matrices and those assembled separately in five color spaces. Every classifier except SVM posts a positive AUC delta; Decision Tree gains 3.9\%, Extra Trees 1.2\%. Three mechanisms explain the lift: (i) pigment gradients introduce chromatic co‑variation unavailable in gray luminance, (ii) shadows modulate luminance more than chrominance, so chromatic GLCMs are less noise–prone, and (iii) the entropy of a three–channel neighborhood is higher, yielding a richer feature manifold. Notably, the improvements taper for highly regularized kernels, showing that color texture benefits learners capable of higher–order interactions.
%
%\paragraph{Binary \emph{vs.} unary ROC detail.}
%In Figure 4.8 the unary ROC curve of Extra Trees lies marginally below the binary curve between FPR 0.0–0.1 but converges thereafter. The AUC difference is 0.017—a gap that translates to three additional false weeds per hundred images at the spray/no–spray threshold used in Chapter 5. Whether that trade–off is acceptable depends on herbicide cost and collateral damage tolerance; the dissertation’s economic analysis in Chapter 6 suggests it often is.
%
%\subsection{Color‐only classification}
%
%Color was assessed in isolation by holding out all texture and shape attributes. Table 4.6 confirms that color alone is weaker: median AUC across models is 0.77; only Extra Trees, Gradient Boosting, and MLP exceed 0.80. The ROC surfaces in Figures 4.13–4.14 illustrate a flatter rise near the origin, reflecting heavy overlap in hue and saturation between crops and weeds cultivated under identical irrigation and fertilization regimes. The parameter convergence noted in Table 4.7 is striking: every classifier chooses exactly \texttt{cb\_mean}, \texttt{hue}, \texttt{in\_phase}, and \texttt{saturation\_mean}. This homogeny indicates that most global color information is redundant once soil pixels are removed, reinforcing the agronomic intuition that ``green is green’’ during early vegetative stages.
%
%\paragraph{Temporal stability.}
%Table 4.8 flags hue and saturation as non‑stationary (ADF $\alpha=0.05$); their drift over a three–week window suggests a phenological confound. Practically, models keyed too heavily to these variables risk seasonally induced degradation unless re‑calibrated. The dissociation between stationarity and discriminative power also explains why color cannot match texture: half the color features fluctuate yet fail to separate classes, while texture variables show higher persistence and greater class separation.
%
%\subsection{Shape‐only classification}
%
%Shape descriptors—convexity, elongation, roundness, solidity, bending energy, compactness—were extracted only for fully contained silhouettes to avoid edge truncation. Table 4.9 shows Gradient Boosting topping the chart at AUC 0.86; Decision Tree bottoms out at 0.73. Ensemble methods again dominate, leveraging nonlinear combinations of curvature and aspect ratio. Shape ROC curves lag texture at low FPRs: thin cotyledons of cantaloupe mimic grass weeds in outline, confusing classifiers until true leaves emerge. Nevertheless, shape features carry orthogonal information, a fact underscored by their recurrence in fused models discussed next.
%
%\subsection{Fused model: integrating color, texture, and shape}
%
%Pooling all 200–plus features yields the highest scores of the study. Table 4.12 reports AUC 0.92 for Extra Trees, Gradient Boosting, and Random Forest; even Logistic Regression clears 0.88. Table 4.13 reveals that none of the best five–feature subsets include pure color statistics. Instead, color enters indirectly as chromatic texture (\texttt{cie\_b\_lbp\_variance}, \texttt{ycbcr\_y\_lbp\_stddev}), validating the hypothesis that hue information is most effective when captured at a spatial rather than per‑pixel scale. Figures 4.17–4.19 confirm visual dominance across the ROC spectrum. The unary and one‑class curves (Figures 4.18–4.19) almost overlap for ensemble learners, reinforcing that a learned ``crop manifold’’ suffices when color, shape, and texture jointly constrain the feature space.
%
%\subsection{Interpreting the ROC curves and the predominance of AUC}
%
%AUC is the integral of sensitivity over the entire false–positive axis and thus agnostic to operating threshold. In agricultural robotics thresholds migrate with weed pressure and economic factors, so AUC provides a stable reference. The dissertation supplements each ROC family with $F_{1}$, precision, and recall (Tables 4.2, 4.6, 4.9, 4.12), but those scalars assume a single threshold and skew with class imbalance. For instance, Decision Tree’s $F_{1}$ on the full feature set is 0.82, nearly matching Extra Trees, yet its ROC curve decays sharply beyond FPR 0.15, indicating brittle performance once the threshold shifts. AUC captures that brittleness; $F_{1}$ does not.
%
%\subsection{Feature selection and the ``flat–top’’ plateau}
%
%Table 4.14 quantifies how many feature subsets land within 3,% of the peak score. For Random Forest 16.22,% of tested subsets are near–optimal; for SVM the proportion rises to 22.52,%. Two forces widen this plateau:
%
%\begin{enumerate}
%\item \textbf{Redundancy.} Many descriptors are correlated (e.g.\ \texttt{greyscale\_contrast\_avg} vs.\ \texttt{greyscale\_dissimilarity\_avg}). Swapping one for its sibling leaves decision boundaries intact; hence dozens of subsets tie for first place.
%\item \textbf{Ensemble averaging.} Extra Trees and Gradient Boosting subsample features at each split. Once the ensemble reaches its bias–variance sweet spot, dropping a modestly informative variable merely re‑weights split probabilities without altering the macro decision surface.
%\end{enumerate}
%
%The practical upshot is welcome: resource‐constrained field computers can prune the feature extractor aggressively—sometimes by half—without losing more than 0.02 AUC. This aligns with observations in Table 5.1 that only six of nine models ever reference \texttt{hog\_mean} despite its frequent appearance in importance rankings.
%
%\subsection{Why color–space texture beats grayscale texture}
%
%Returning to Table 4.3, we now tie the empirical gains to physical leaf properties. Chromatic GLCMs benefit from ``spectral decorrelation’’: pigments such as carotenoids make weeds slightly yellower, altering the b axis in CIELab; thick waxy cuticles reflect more in the blue, nudging YCbCr’s Cb channel. Computing co‑occurrence on those channels surfaces subtle periodicities invisible in gray. Shadow noise is also compartmentalized: illumination changes primarily affect luminance, so chrominance pairs maintain higher signal–to–noise. Finally, the state space triples, elevating the entropy baseline of second–order statistics; texture measures that penalize uniformity (e.g.\ inverse difference moment) become more discriminative. These effects collectively explain the consistent but modest uptick in AUC across classifiers.
%
%\subsection{The marginal returns of additional hyper–parameters}
%
%Table 4.15 lists optimum probability thresholds: they cluster tightly between 0.49 and 0.58 for every learner except Decision Tree (fixed at 1.00). That uniformity indicates well‑calibrated posterior estimates and implies that further calibration (Platt scaling, isotonic regression) would shift operating points by at most a point or two. Similarly, grid search logs show that deepening trees or adding booster stages lifts ensemble AUC by under 0.5,%. The diminishing returns echo the earlier feature–selection plateau: once a rich descriptor bank is available, algorithmic fine‑tuning tweaks rather than transforms performance.
%
%\subsection{Correcting class imbalance: oversampling and hybrid methods}
%
%The training corpus exhibits a worst–case crop:weed ratio of 50:1. Section 4.2 subjects nine classifiers to five imbalance levels and three resampling regimes. Figure3 (ADASYN) and Figure4 (SMOTE+Tomek, SMOTE+ENN) reveal that oversampling generally nudges ROC curves upward, but rarely more than a millimeter on the printout. Decision Tree is the standout beneficiary: AUC rises from 0.84 to 0.89 after SMOTE+ENN at the highest imbalance, nearly matching its balanced baseline. Ensemble learners gain little; Random Forest’s curve is almost unchanged, and Gradient Boosting sees a visible lift only with Tomek links that excise mislabeled crops.
%
%These patterns align with theory: tree ensembles already diversify decision boundaries; synthetic weeds add little new information and can even inject noise. Simpler axis–aligned trees, by contrast, overfit the majority; rebalancing mitigates that tendency. Logistic Regression and SVM show mixed results—small gains at high imbalance, slight dips at mild imbalance—reflecting the delicate trade‑off between margin width and synthetic outliers.
%
%\paragraph{Precision–recall consequences.}
%While AUC remains king, the dissertation wisely reports precision and recall. Oversampling typically inflates recall (more weeds found) and deflates precision (more crops mistaken). Decision Tree’s weed recall jumps from 0.25 to 0.75 in the illustrative confusion matrices of Figure 9, while precision falls from 0.71 to 0.60, yet $F_{1}$ climbs from 0.37 to 0.67, corroborating that recall gains dominate. For herbicide sprayers, that trade‑off is often acceptable because false negatives (missed weeds) incur future yield loss, whereas false positives waste chemical but rarely kill crops thanks to selective herbicides.
%
%\subsection{Synthesis, agronomic implications, and guidelines}
%
%Three high‑level trends emerge:
%
%\begin{enumerate}
%\item \textbf{Cue hierarchy.} Texture 
%>
%> shape 
%>
%> color in standalone effectiveness. This hierarchy mirrors botanic reality: venation and surface roughness are species‐specific and stable across lighting; silhouette evolves with ontogeny; color is volatile with nutrition and stress.
%\item \textbf{Redundancy as a feature, not a bug.} The flat‑top plateau means engineers can substitute cheaper or faster descriptors without a catastrophic hit. For embedded GPUs the dissertation recommends a five–feature subset (\texttt{cie\_b\_lbp\_variance}, \texttt{greyscale\_contrast\_avg}, \texttt{hog\_mean}, \texttt{roundness}, \texttt{saturation\_mean}) that sustains AUC 0.90 while shrinking inference time four‑fold (Chapter 6 benchmarking).
%\item \textbf{Unary deployment.} One–class KNN on the fused set achieves AUC 0.88—only two points shy of the binary leader yet requiring \emph{zero} weed labels. This is transformative for growers rotating to new weed spectra each season.
%\end{enumerate}
%
%\subsection{Limitations and future directions}
%
%Several caveats warrant mention. First, images were captured under desert sun at nadir; oblique or cloudy conditions might erode chromatic texture gains. Second, the study omits hyperspectral bands beyond RGB and NIR; early experiments with shortwave infrared show promise for discriminating waxy pigmented weeds. Third, phenological drift in color and shape suggests a multi‑task learner that incorporates growth stage could stabilize performance. Finally, occasional segmentation errors noted in Figure 4.1 propagate into misclassification; integrating instance segmentation and classification into a single end‑to‑end network could reduce that error mode.
%
%\subsection{Conclusion}
%
%The dissertation convincingly demonstrates that robust weed/crop discrimination in early growth hinges on multispectral texture descriptors enriched by chromatic information, tempered by ensemble classifiers resilient to feature redundancy. AUC analysis across ROC curves validates that texture extracted in color space consistently edges gray‑only texture, that feature selection enjoys a forgiving performance plateau, and that imbalance corrections matter primarily for simpler learners. By cross‑linking agronomic insight with machine–learning evidence, the work delivers actionable guidelines: fuse cues whenever compute permits, prefer unary models when weeds are unlabeled, and exploit the plateau to tailor descriptor budgets to hardware constraints. These lessons should generalize to other row crops provided segmentation quality is upheld and the texture bank is re‑calibrated for canopy geometry and sensor resolution.
%%%%%%%%%%%%
...
% The plot was generated with this command:
% python plot-depth.py --input c:/uofa/maricopa/raw/2024-05-01/drip-depth/depth_004.npy
% This plot was manually saved after selecting ``tight layout'' and adjusting the margins
%
% The RGB image is the corresponding JPG in the same directory

\begin{figure}[H]
	\centering
	\subfloat[RGB Image of Cantaloupe]{\includegraphics[width=.48\linewidth]{./figures/depth-rgb.jpg}\label{fig:depth-rgb}}
	\hfill
	\subfloat[Depth Data for RGB Image]{\includegraphics[width=.48\linewidth]{./figures/depth-data.png}\label{fig:depth-data}}
...
\includepdf[pages=-]{./mdpi/segmentation-mdpi-revised.pdf}
\newpage
\chapter{Journal Paper: \textit{Class Imbalance in Crop/Weed Classification}}
\includepdf[pages=-]{./mdpi-imbalance/imbalance-mdpi-revised.pdf}
